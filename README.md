...
- **Warehouse connections.** Postgres, BigQuery, Snowflake, Amazon Redshift, Databricks and Microsoft SQL Server, connected in the web app so credentials never pass through your AI. Which of them a refreshing dashboard can publish against is read out of the database as the publish is judged, so the publish refusal names the current set and this sentence names a reading: on 2026-09-11 it was BigQuery, Databricks, Postgres, SQL Server and Snowflake. Redshift can be connected, explored and validated, but not published against.
